...
def main(font: Font) -> None:
    ...
    for count, instance in enumerate(instances, start=1):
        subfamily_name = f"{instance.subfamilyNameID}: {font.t_name.table.getDebugName(instance.subfamilyNameID)}"
        postscript_name = f"{instance.postscriptNameID}: {font.t_name.table.getDebugName(instance.postscriptNameID)}"
        table.add_row(
            str(count),
